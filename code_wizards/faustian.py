...
class Faustian(Wizard):
    ...
    def read_the_stars(self):
        ...
        if 'Aries' in locations_with_cards:
            locations_with_cards[
                "The monks and pilgrims of the Temples <i> (Aries, Hierophant) </i>"] = locations_with_cards.pop(
                'Aries')
        if 'Taurus' in locations_with_cards:
            locations_with_cards[
                "The merchants and bankers of the Blue City <i> (Taurus, Hierophant) </i>"] = locations_with_cards.pop(
                'Taurus')
        if 'Gemini' in locations_with_cards:
            locations_with_cards[
                "The farmers and shepherds of the Chalk Cliffs <i> (Gemini, Hierophant) </i>"] = locations_with_cards.pop(
                'Gemini')
        if 'Cancer' in locations_with_cards:
            locations_with_cards[
                "The beggars and thieves of Scuttleport <i> (Cancer, Hierophant) </i>"] = locations_with_cards.pop(
                'Cancer')
        if 'Leo' in locations_with_cards:
            locations_with_cards[
                "The lords and ladies of the Noble Clans <i> (Leo, Warlock) </i>"] = locations_with_cards.pop('Leo')
        if 'Virgo' in locations_with_cards:
            locations_with_cards[
                "The soldiers and servants of the Halcyon Isles <i> Virgo, Warlock </i>"] = locations_with_cards.pop(
                'Virgo')
        if 'Libra' in locations_with_cards:
            locations_with_cards[
                "The fishermen and divers of the Far Reach <i> Libra, Mariner </i>"] = locations_with_cards.pop('Libra')
        if 'Scorpio' in locations_with_cards:
            locations_with_cards[
                "The sailors and travelers beyond Isha <i> Scorpio, Mariner </i>"] = locations_with_cards.pop('Scorpio')
        if 'Sagittarius' in locations_with_cards:
            locations_with_cards[
                "The students and professors of Spyrholm University <i> Sagittarius, Sorcerer </i>"] = locations_with_cards.pop(
                'Sagittarius')
        if 'Capricorn' in locations_with_cards:
            locations_with_cards[
                "The researchers and scholars of the Ancient Archives <i> Capricorn, Sorcerer </i>"] = locations_with_cards.pop(
                'Capricorn')
        if 'Aquarius' in locations_with_cards:
            locations_with_cards[
                "The druids and hermits of the Starlit Atoll <i> Aquarius, Sage </i>"] = locations_with_cards.pop(
                'Aquarius')
        if 'Pisces' in locations_with_cards:
            locations_with_cards[
                "The dead and nearly dead of the Graven Isle <i> Pisces, Necromancer </i>"] = locations_with_cards.pop(
                'Pisces')

        mercury_alone_html = ""
        card_string = "%s" % "".join(
            ["<h2>%s</h2> now have %s schemes of the devil in their midst.<br>" % (k, locations_with_cards[k]) for k in
             sorted(locations_with_cards)])
        mercury_among_html = ""
        if magic_number == 0:
            mercury_alone_html = """
